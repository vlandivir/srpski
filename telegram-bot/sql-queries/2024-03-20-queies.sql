...
candidate_cards as (
    select c.image, c.generated_at, ucr.card_weight, ucr.user_id, ucr.created_at,
        ...
        from prod_cards c
        left join last_card_responses ucr
        on c.image = ucr.card_image and ucr.user_id = '807625513'
        order by generated_at
)
select *
    from candidate_cards
...
select * from prod_user_card_responses order by created_at desc limit 20;

defaultdb=> select distinct user_id from prod_user_card_responses;
  user_id
-----------
 150847737
 807625513
(2 rows)
        ...
        WITH ranked_responses AS (
            SELECT
                ...
                ROW_NUMBER() OVER (PARTITION BY user_id, card_image, DATE(created_at) ORDER BY created_at DESC) AS rank
            FROM prod_user_card_responses
            WHERE user_id = '807625513' AND DATE(created_at) BETWEEN CURRENT_DATE - INTERVAL '1 day' AND CURRENT_DATE
        ),
        filtered_responses AS (
            SELECT
                response_date,
        ...
        SELECT TO_CHAR(response_date, 'YYYY-MM-DD') as response_date,
